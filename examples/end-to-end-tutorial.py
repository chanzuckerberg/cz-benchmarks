...
print(f"Labels available: {dataset.labels.name} with {len(dataset.labels.unique())} unique values.")
...
fig2 = plot_umap(clustering_baseline_embedding, 'UMAP of PCA Baseline Embedding', dataset.labels)
...
embedding_results_baseline = embedding_task.run(embedding_baseline_embedding, embedding_task_input)
...
prediction_results_baseline = prediction_task.run(prediction_baseline_embedding, prediction_task_input)
...
sns.barplot(data=df_pred_overall, x='metric_name', y='value', hue='source', palette="magma")
plt.title('Metadata Label Prediction Performance: scVI Model vs. PCA Baseline', fontsize=16)
plt.ylabel('Score', fontsize=12)
plt.xlabel('Metric', fontsize=12)
