...
# FIXME MICHELLE fix column names after release
...
def _compute_de_wilcoxon(
    ...
    *,
    ...
    control_cells_ids = (
        adata.uns.get(UNS_KEY_CONTROL_CELLS_IDS) if hasattr(adata, "uns") else None
    )
    ...
    if len(control_cells_ids) == 0:
        return pd.DataFrame(
            columns=["condition", "gene", "logfoldchange", "pval_adj"]
        ).copy()
    ...
    # FIXME MICHELLE: this is the rename that changes it
...
def get_matched_controls(
    ...
    *,
    ...
    if any(
        field is None
        for field in [pert_column, ctrl_condition, gem_column, libsize_column]
    ):
        raise ValueError(
            "All column fields (pert_column, ctrl_condition, gem_column, libsize_column) must be provided"
        )
    ...
    for group_name, group in groups:
        ...
        if (
            gem_column is not None
            and gem_column in ctrl_df.columns
        ...
        if len(available_ctrls) == 0:
            print(
                f"Warning: No unused controls available in group {group_name if group_name is not None else 'ALL'} for condition {condition}"
            )
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--input_path", required=True, help="Path to a raw .h5ad file to process"
    )
    p.add_argument(
        "--output_dir",
        required=True,
        help="Directory to write the processed .h5ad file",
    )
    p.add_argument(
        "--condition_col",
        default="condition",
        help="Obs column name for perturbation condition",
    )
    p.add_argument(
        "--control_name",
        default="non-targeting",
        help="Control group name in condition column",
    )
    p.add_argument(
        "--filter_min_cells",
        type=int,
        default=10,
        help="Minimum cells per gene for filtering before DGE",
    )
    p.add_argument(
        "--filter_min_genes",
        type=int,
        default=1000,
        help="Minimum genes per cell for filtering before DGE",
    )
    p.add_argument(
        "--min_pert_cells",
        type=int,
        default=10,
        help="Minimum perturbed cells required per condition",
    )
    return p.parse_args()
