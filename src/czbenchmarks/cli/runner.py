...
from .resolve_reference import is_anndata_reference, resolve_anndata_references
...
def run_task(
    ...
    *,
    ...
    if is_anndata_reference(cell_representation):
        model_representation = resolve_anndata_references(cell_representation, adata)

    # Resolve any AnnData references within the task parameters
    task_params_resolved = resolve_anndata_references(task_params, adata)

    # Validate and create the TaskInput object
    validated_input = TASK_REGISTRY.validate_task_inputs(
        task_name, task_params_resolved
    )
    # If validation returns a Pydantic model, use it; otherwise create one
    if isinstance(validated_input, InputModel):
        task_input = validated_input
    else:
        try:
    ...
    if run_baseline:
        ...
        try:
            ...
            baseline_params_resolved = resolve_anndata_references(
                baseline_params, adata
            )
            # Validate and create the BaselineInput object
            validated_baseline = TASK_REGISTRY.validate_baseline_inputs(
                task_name, baseline_params_resolved
            )
            if isinstance(validated_baseline, BaselineModel):
                baseline_input = validated_baseline
            else:
