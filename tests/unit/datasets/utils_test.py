        ...
        mock.return_value = OmegaConf.create(
            {
                "datasets": {
                    ...
                    "s3_dataset": {
                        "_target_": "czibench.datasets.sc.SingleCellDataset",
                        "path": "s3://my-bucket/s3_test_dataset.h5ad",
                        "organism": "HUMAN",
                    },
                }
            }
...
def mock_instantiate():
    with patch("czibench.datasets.utils.instantiate") as mock:
        mock.return_value = SingleCellDataset(
            path="/tmp/test_dataset.h5ad", organism="HUMAN"
        )
        yield mock
...
def test_download_dataset_creates_directories(mock_s3_client, tmp_path):
    local_file = tmp_path / "nested/dir/my-dataset.h5ad"
    mock_s3_client.download_file.return_value = None
    _download_dataset("s3://my-bucket/my-dataset.h5ad", str(local_file))
    assert local_file.parent.exists()


def test_load_dataset_invalid_name(
    mock_hydra, mock_hydra_initialize, mock_hydra_compose
):
