...
def test_cli_e2e_workflow(mocker: MockFixture):
    ...
    assert isinstance(embedding_results, dict), (
        "Embedding results should be a dictionary"
    )
    assert ModelType[model_name] in embedding_results, (
        f"Embedding results should contain {model_name}"
    )
    embedding_model_results = embedding_results[ModelType[model_name]]
    assert isinstance(embedding_model_results, list), (
        "Embedding model results should be a list"
    )
    assert len(embedding_model_results) > 0, (
        "Embedding model results should not be empty"
    )
    assert all(isinstance(r, MetricResult) for r in embedding_model_results), (
        "All embedding results should be MetricResult objects"
    )
    ...
    assert isinstance(clustering_results, dict), (
        "Clustering results should be a dictionary"
    )
    assert ModelType[model_name] in clustering_results, (
        f"Clustering results should contain {model_name}"
    )
    clustering_model_results = clustering_results[ModelType[model_name]]
    assert isinstance(clustering_model_results, list), (
        "Clustering model results should be a list"
    )
    assert len(clustering_model_results) > 0, (
        "Clustering model results should not be empty"
    )
    assert all(isinstance(r, MetricResult) for r in clustering_model_results), (
        "All clustering results should be MetricResult objects"
    )
    ...
    assert isinstance(prediction_results, dict), (
        "Prediction results should be a dictionary"
    )
    assert ModelType[model_name] in prediction_results, (
        f"Prediction results should contain {model_name}"
    )
    prediction_model_results = prediction_results[ModelType[model_name]]
    assert isinstance(prediction_model_results, list), (
        "Prediction model results should be a list"
    )
    assert len(prediction_model_results) > 0, (
        "Prediction model results should not be empty"
    )
    assert all(isinstance(r, MetricResult) for r in prediction_model_results), (
        "All prediction results should be MetricResult objects"
    )
