...
def test_model_to_display_name():
    ...
    assert ("Tester", "") == utils.model_to_display(dummy_model_type, {})
    ...
    for k, v in utils._MODEL_VARIANT_FINETUNE_TO_DISPLAY.items():
        dummy_model_type = DummyModelType(k[0])
        args = {"model_variant": k[1]}
        if k[2] is not None:
            args["dataset"] = k[2]
        assert v == utils.model_to_display(dummy_model_type, args)
