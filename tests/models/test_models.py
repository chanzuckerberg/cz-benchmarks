...
from czbenchmarks.datasets.single_cell import SingleCellDataset, PerturbationSingleCellDataset
...
def test_valid_dataset(validator_class, obs_columns, var_columns, dataset_class):
    ...
    if dataset_class == PerturbationSingleCellDataset:
        dummy_truth = {"ctrl": pd.DataFrame(np.ones((ann.n_obs, ann.n_vars)), columns=ann.var_names)}
...
def test_invalid_dataset_type():
    ...
    ann = create_dummy_anndata(obs_columns=["dataset_id", "assay", "suspension_type", "donor_id"])
...
def test_missing_required_obs_keys():
    validator = SCVIValidator()  # Requires obs keys: dataset_id, assay, suspension_type, donor_id.
