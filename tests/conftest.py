...
# Define fixtures and other configuration options that can be shared across multiple test files
...
def create_dummy_anndata_fixture():
    def _make_data(n_cells=5, n_genes=3, obs_columns=None, var_columns=None):
        return create_dummy_anndata(n_cells=n_cells, n_genes=n_genes,
...
def dummy_human_anndata(tmp_path):
    ...
    var = pd.DataFrame({"ensembl_id": gene_names, "other": gene_names}, index=gene_names)
    adata = ad.AnnData(X=X, obs=obs, var=var)
    adata.write_h5ad(file_path)
    return str(file_path)

# Creates a dummy AnnData object with invalid gene name prefixes and saves it to a temporary file.
@pytest.fixture
