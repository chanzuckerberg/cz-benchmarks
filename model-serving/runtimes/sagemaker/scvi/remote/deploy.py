...
def create_model_package_group(sm_client, group_name, description):
    ...
    try:
        response = sm_client.describe_model_package_group(
            ModelPackageGroupName="scvi-model-package-group"
        )
        model_package_group_arn = response["ModelPackageGroupArn"]
        logger.info(
            f"Retrieved existing Model Package Group ARN: {model_package_group_arn}"
        )
        return model_package_group_arn
    except ClientError:
        response = sm_client.create_model_package_group(
            ModelPackageGroupName=group_name, ModelPackageGroupDescription=description
        )
        model_package_group_arn = response["ModelPackageGroupArn"]
        logger.info(
...
def create_or_update_endpoint(sm_client, endpoint_name: str, endpoint_config_name: str):
    ...
    if endpoint_exists(endpoint_name, sm_client):
        logger.info(
            f"Endpoint '{endpoint_name}' already exists. Updating the existing endpoint."
        )
        response = sm_client.update_endpoint(
            EndpointName=endpoint_name, EndpointConfigName=endpoint_config_name
        )
        logger.info(
            f"Endpoint '{endpoint_name}' updated with arn: {response['EndpointArn']}"
        )
    else:
        logger.info(
            f"Endpoint '{endpoint_name}' does not exist. Creating a new endpoint."
        )
        response = sm_client.create_endpoint(
            EndpointName=endpoint_name, EndpointConfigName=endpoint_config_name
        )
        logger.info(
            f"Endpoint '{endpoint_name}' created with arn: {response['EndpointArn']}"
        )
